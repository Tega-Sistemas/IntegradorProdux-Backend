...
async function sincronizar() {
    ...
    const syncName = 'motivo retrabalho'
    var data;
    ...
    try {
        ...
        if (configuracao.length > 0) {
            configuracao.forEach(item => {
                urlExterna = `${item.tipo_conexao}://${item.ip}:${item.porta}/${item.nome_aplicacao}/Api/IntegracaoProdux/motivoretrabalho`;
            });
        } else {
        ...
        const select = await MotivoRetrabalho.query().select('MotivoRetrabalhoId', 'MotivoRetrabalhoDescricao');

        data = {
            "MotivoRetrabalho": select
        };
