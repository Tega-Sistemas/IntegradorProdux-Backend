    ...
    try {
        ...
        if (configuracao.length > 0) {
            configuracao.forEach(item => {
                urlExterna = `${item.tipo_conexao}://${item.ip}:${item.porta}/${item.nome_aplicacao}/Api/IntegracaoProdux/motivoparada`;
            });
        } else {
        ...
        const select = await MotivoParada.query().select('MotivoParadaId', 'MotivoParadaDescricao', 'MotivoParadaEvitavel');
